    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-blue-50 via-white to-purple-50 px-4">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm sm:text-base text-gray-600">Validating your reset link...</p>
        </div>
      </div>
    );
  }
      ...
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <Link to="/login" className="text-gray-600 hover:text-blue-600 text-xs sm:text-sm min-h-11 flex items-center">
              Back to Login
            </Link>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-3 sm:px-4 py-6 sm:py-8">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
          ...
          <div className="bg-linear-to-r from-blue-600 to-purple-600 px-5 sm:px-8 py-5 sm:py-6 text-white">
            <h1 className="text-xl sm:text-2xl font-bold">Reset Admin Password</h1>
            <p className="text-blue-100 text-xs sm:text-sm mt-1">Create a new password for your admin account</p>
          </div>

          <div className="p-5 sm:p-8">
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-700 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg mb-4 text-xs sm:text-sm flex items-start gap-2">
            ...
            {success && (
              <div className="bg-green-50 border border-green-200 text-green-700 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg mb-4 text-xs sm:text-sm flex items-start gap-2">
                <span className="text-base sm:text-lg">✅</span>
                <span>{success}</span>
              </div>
            )}
            ...
            {!isTokenValid ? (
              <div className="text-center py-6 sm:py-8">
                ...
                <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">Invalid or Expired Link</h3>
                ...
                <Link 
                  to="/login" 
                  className="inline-block bg-blue-600 text-white px-5 sm:px-6 py-2 rounded-lg hover:bg-blue-700 transition text-sm sm:text-base min-h-11"
                >
                  Back to Login
                </Link>
              </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-400 mt-1">Must be at least 6 characters</p>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-linear-to-r from-blue-600 to-purple-600 text-white py-3 rounded-lg font-semibold hover:from-blue-700 hover:to-purple-700 transition shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base min-h-13"
                >
                  {loading ? (
                ...
                <div className="mt-6 text-center">
                  <Link to="/login" className="text-xs sm:text-sm text-blue-600 hover:text-blue-800 hover:underline transition min-h-11 inline-flex items-center">
                    ← Back to Login
                  </Link>
                </div>
              </form>
      ...
      <footer className="bg-white border-t py-3 sm:py-4">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 flex flex-col sm:flex-row justify-between items-center text-[10px] sm:text-sm text-gray-500">
          ...
          <div className="flex gap-4 sm:gap-6 mt-2 sm:mt-0">
            ...
            <Link to="/login" className="hover:text-blue-600 transition min-h-11 flex items-center">Login</Link>
          </div>
        </div>
      </footer>
    </div>
